...
REM If server is already running, do not start a second instance
powershell -NoProfile -Command "try { $r = Invoke-WebRequest -UseBasicParsing -Uri ('http://localhost:' + %PORT% + '/api/health') -TimeoutSec 2; if ($r.StatusCode -ge 200 -and $r.StatusCode -lt 500) { exit 0 } else { exit 1 } } catch { exit 1 }"
if not errorlevel 1 (
